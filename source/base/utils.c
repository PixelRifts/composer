...
int g_frame_arena_i = 0;

void U_FrameArenaInit(void) {
	arena_init(&frame_arena[0]);
    arena_init(&frame_arena[1]);
    g_frame_arena_i = 0;
}

void U_FrameArenaFree(void) {
	arena_free(&frame_arena[1]);
    arena_free(&frame_arena[0]);
}

M_Arena* U_GetFrameArena(void) {
	return &frame_arena[g_frame_arena_i];
}

void U_ResetFrameArena(void) {
	g_frame_arena_i = !g_frame_arena_i;
    arena_dealloc_to(&frame_arena[g_frame_arena_i], 0);
}
